...
const useHalMetadata = (halUrl, options = {}) => {
  ...
  const fetchWithRetry = useCallback(
    async (url, attempt = 1) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `API HAL erreur ${response.status}: ${response.statusText}`,
          );
        }
  ...
  const transformMetadata = useCallback(rawData => {
    ...
    return {
      ...
      year: doc.submittedDate_s
        ? new Date(doc.submittedDate_s).getFullYear()
        : null,
    };
  }, []);
  ...
  const fetchMetadata = useCallback(
    async (force = false) => {
      ...
      try {
        ...
        console.error(
          '[useHalMetadata] Erreur récupération métadonnées:',
          error,
        );
