  ...
  SUPPORTED_LOCALES.forEach(locale => {
    ...
    if (fs.existsSync(localeDir)) {
      ...
      files.forEach(file => {
        ...
        try {
          translations[locale][namespace] = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        } catch (err) {
          log(`Erreur lors du chargement de ${filePath}: ${err.message}`, 'red');
        }
      });
    }
  });
...
function analyzeTranslations() {
  ...
  const allUsedKeys = [...new Set([...usedKeysInPages, ...usedKeysInComponents])];
  ...
  SUPPORTED_LOCALES.forEach(locale => {
    ...
    if (unusedKeys[locale].length > 0) {
      log(`\n  ⚠️  Clés non utilisées (${unusedKeys[locale].length}):`, 'yellow');
  ...
  if (onlyInFr.length > 0) {
    log(`\n  ⚠️  Clés présentes uniquement en FR (${onlyInFr.length}):`, 'yellow');
  ...
  if (onlyInEn.length > 0) {
    log(`\n  ⚠️  Clés présentes uniquement en EN (${onlyInEn.length}):`, 'yellow');
  ...
  const reportPath = path.join(__dirname, `translation-audit-${Date.now()}.json`);
