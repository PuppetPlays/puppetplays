...
if (auditFiles.length === 0) {
  console.error(
    "Aucun rapport d'audit trouvé. Exécutez d'abord audit-translations.js",
  );
  process.exit(1);
}
...
function createTranslationStructure(key, locale) {
  ...
  const translations = {
    fr: {
      ...
      'meta.animationTechniques.description':
      ...
      authors: 'Auteurs',
      'meta.authors.description':
        'Découvrez les auteurs du théâtre de marionnettes',
      ...
      'meta.discoveryPaths.description':
        'Explorez nos parcours de découverte thématiques',
...
['fr', 'en'].forEach(locale => {
  ...
  console.log(
    `${missingKeys.length} clés valides à traduire (sur ${audit.missingKeys[locale].length} au total)`,
  );
  ...
  if (missingKeys.length > 0) {
    ...
    Object.entries(grouped).forEach(([namespace, translations]) => {
      ...
      console.log(
        `  ✅ Créé: missing-translations/${locale}/${namespace}.json`,
      );
    });
  }
});
...
console.log(
  '1. Vérifiez les traductions générées dans scripts/missing-translations/',
);
console.log('2. Ajustez les traductions si nécessaire');
console.log(
  '3. Fusionnez les traductions avec les fichiers existants dans public/locales/',
);
